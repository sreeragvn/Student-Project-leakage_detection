...
pastcases=np.zeros((349*(lastday-7),7))
indexcases=0
for index in range(0,349):
    cases = df_cases['DailyCases'].iloc[index*lastday:(index+1)*lastday].reset_index(drop=True)
    for i in range (0,lastday-7):
        counter=0
        for j in range (0,7):
            pastcases[indexcases,counter]=cases[i+j]
            counter=counter+1
        indexcases=indexcases+1

df_cases=df_cases[df_cases['Day'] > 6].reset_index(drop=True)
temp=pd.DataFrame(pastcases)
df_cases[['0','1','2','3','4','5','6']]=temp[[0,1,2,3,4,5,6]]
...
pastdeaths=np.zeros((349*(lastday-7),7))
indexcases=0
for index in range(0,349):
    deaths = df_death['DailyDeaths'].iloc[index*lastday:(index+1)*lastday].reset_index(drop=True)
    for i in range (0,lastday-7):
        counter=0
        for j in range (0,7):
            pastdeaths[indexcases,counter]=deaths[i+j]
            counter=counter+1
        indexcases=indexcases+1

df_death=df_death[df_death['Day'] > 6].reset_index(drop=True)
temp=pd.DataFrame(pastdeaths)
df_death[['0','1','2','3','4','5','6']]=temp[[0,1,2,3,4,5,6]]
...
pastrecs=np.zeros((349*(lastday-7),7))
indexcases=0
for index in range(0,349):
    recs = df_rec['DailyRecovery'].iloc[index*lastday:(index+1)*lastday].reset_index(drop=True)
    for i in range (0,lastday-7):
        counter=0
        for j in range (0,7):
            pastrecs[indexcases,counter]=recs[i+j]
            counter=counter+1
        indexcases=indexcases+1

df_rec=df_rec[df_rec['Day'] > 6].reset_index(drop=True)
temp=pd.DataFrame(pastrecs)
df_rec[['0','1','2','3','4','5','6']]=temp[[0,1,2,3,4,5,6]]

################################################################################################

cols= df_cases.columns
cols=cols[:-7]
df_active=df_cases[cols].copy()
...
df_active[['0','1','2','3','4','5','6']]=df_cases[['0','1','2','3','4','5','6']]-(df_death[['0','1','2','3','4','5','6']]+df_rec[['0','1','2','3','4','5','6']])

df_active.to_csv('DaySeriesActiveCovidDataSetWithPast.csv', header=True, index=False, encoding='utf-8')
