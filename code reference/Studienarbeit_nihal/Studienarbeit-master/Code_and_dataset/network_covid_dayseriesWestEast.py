...
def main():
    ...
    target = dfW[['RelativeLogDailycases','west(0)/east(1)']]
    ...
    testtarget = dfE[['RelativeLogDailycases','west(0)/east(1)']]
    testtarget = testtarget.values
    
    model = keras.models.Sequential()
    # model.add(keras.layers.Dropout(0.2,input_shape=(6,)))
    model.add(keras.layers.Dense(50,activation=None,input_dim=5,kernel_initializer='normal',kernel_constraint=max_norm(5)))
    for i in range(0,15):
    ...
    history = model.fit(x=features,y=target[:,:1],batch_size=100 ,epochs=20)#,validation_split=0.1)
    ...
    model_json = model.to_json()
    with open("AgeGroupOmitBigCities/WestLogRelative.json", "w") as json_file:
        json_file.write(model_json)
    # serialize weights to HDF5
    model.save_weights("AgeGroupOmitBigCities/WestLogRelative.h5")
    ...
    for i in range(0,dfE.shape[0]/lastday):
        ...
        ax.legend()   
        filename='AgeGroupOmitBigCities/plotlogWestEastRelative/'+dfE['Landkreis'].iloc[i*lastday]+'.png'
