...
def main():
    ...
    lastday=129
    df=pd.read_csv('DaySeriesActiveCovidDataSet.csv')
    
    target = df[['ActiveCases','west(0)/east(1)']]
    target = target.values
    features = df[['Density','Income','<30','30-65','>65','firstday','Day']]
    features = features.values
    
    df=df.sort_values(['Landkreis','Day']).reset_index(drop=True)
    testfeatures = df[['Density','Income','<30','30-65','>65','firstday','Day']]
    ...
    for i in range(0,349):
        ...
        fig, ax = plt.subplots()
        ax.plot(testfeatures[i*lastday:(i+1)*lastday,6],testtarget[i*lastday:(i+1)*lastday,0])
        ax.plot(testfeatures[i*lastday:(i+1)*lastday,6],target_predict)
        filename='plots_active/'+str(int(df['west(0)/east(1)'].iloc[i*lastday]))+df['Landkreis'].iloc[i*lastday]+'.png'
